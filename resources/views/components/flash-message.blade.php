@if($message = Session::get('success'))
    <div x-data="{ open: true }" x-show="open" class="mb-4 p-4 bg-emerald-50 border border-emerald-200 rounded-lg flex justify-between items-center">
        <div class="flex items-center gap-3">
            <span class="text-emerald-600 text-lg">✓</span>
            <p class="text-sm font-medium text-emerald-800">{{ $message }}</p>
        </div>
        <button @click="open = false" class="text-emerald-600 hover:text-emerald-800">✕</button>
    </div>
@endif
